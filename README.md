...
It can also be values such as: IDLE_HOST, DLE_HOST+PENDING_JOB, APP_FINISHED or CLOSE_TO_DEADLINE in the case of event generated by monitoring the infrastructure manager. The last for that can be used specifies application based events, in which the following pattern must be met. 
