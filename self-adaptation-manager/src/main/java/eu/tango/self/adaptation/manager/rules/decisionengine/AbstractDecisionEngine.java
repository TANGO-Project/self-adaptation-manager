...
import eu.tango.self.adaptation.manager.model.Slot;
...
//        VmRequirements requirements = OVFUtils.getVMRequirementsFromOvfType(response.getCause().getApplicationDefinition(), vmOvfType);
...
//            }
//            ApplicationDefinition applicationDefinition = getActuator().getApplication(response.getApplicationId(), response.getDeploymentId(), vmId + "");
//            if (currentValue > answerPower && (vmType == null || applicationDefinition.getOvfId().equals(vmType))) {
//                answer = vmId;
...
//        }
     ...
     */
    public ArrayList<PowerApplicationMapping> getApplicationPowerList(Response response, List<Integer> vmIds) {
        ...
        for (Integer vmId : vmIds) {
            ...
            ApplicationDefinition application = getActuator().getApplication(response.getApplicationId(), response.getDeploymentId(), vmId + "");
...
//        OvfDefinition ovf = response.getCause().getApplicationDefinition();
